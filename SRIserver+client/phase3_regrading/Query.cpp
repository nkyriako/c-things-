...
void makeFacts1(std::vector<Fact*>& facts1, RB* rb, KB* kb, Rule* rule ){
  	std::thread::id this_id = std::this_thread::get_id();
        ...
        //std::cout<<"Thread " << this_id << " ended"<<std::endl;
		...
		std::string param2)
{
	...
	//std::thread::id this_id = thread1.get_id();
	//std::cout<<"Thread " << this_id << " started"<<std::endl;
	std::thread thread2(makeFacts2, std::ref(facts2), rb, kb, rule);
	std::thread::id this_id1 = thread2.get_id();
	//std::cout<<"Thread " << this_id1 << " started"<<std::endl;

	//join both threads to main thread 
	thread1.join();
	//std::cout<<"Thread 1 finished"<<std::endl;
	//std::cout<<"Thread " << this_id << " finished"<<std::endl;
	thread2.join();
	//std::cout<<"Thread 2 finished"<<std::endl;
	//std::cout<<"Thread " << this_id1 << " finished"<<std::endl;
	...
	else // op == 'A'
	{
       ...
       // 1 != 3, 2 == 4
    	   ...
    	   if( rule->param1.at(1)==rule->param2.at(0) ){
			...
			{
				//std::cout << "TESTTTTTTTT";
				if(facts1.at(i)->param.at(1).compare(facts2.at(j)->param.at(0)) == 0)
				{
					facts3.push_back(new Fact(rule->Name,facts1.at(i)->param.at(0),facts2.at(j)->param.at(1))); 
			//		std::cout << "TESTER";
				}
			}
        	   }
